...
public class VacuumGame {
	 ...
	 * lengths, and that each character in it (other than newline) is a
	 * character that represents one of the <code>Sprite</code>s in this game.
	 * Uses gridType to implement the grid.
	 * 
	...
	/**
	 * This method will check if the move is valid If the given move is valid
	 * then move the vacuum to the given location based on movement of player
	 * 
	...
	private void validP2Move(Sprite spr) {
		if (spr.getSymbol() == Constants.DUMPSTER) {
			...
			if (vacuum2.clean(Constants.DUST_BALL_SCORE)) {
	...
	private void validP1Move(Sprite spr) {
		if (spr.getSymbol() == Constants.DUMPSTER) {
			...
			if (vacuum1.clean(Constants.DUST_BALL_SCORE)) {
				dustBalls.remove(spr);
			}
		}
	}
	 ...
	 * 
	 * @param symbol
	 *            the Sprite that we want to generate from
	 * @param row
	 *            the row value of symbol
	 * @param column
	 *            the column value of symbol
	 * @return a Sprite
	 */
